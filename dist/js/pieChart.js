...
function animation() {
    ...
    if (j > sum) {
        ...
        stat = stat + '<path id=\"p' + i + '\"  d=\"' + pies[i].start + ' A ' + r + ' ' + r + ' ' + pies[i].end + pies[i].endIn + ' A ' + 0.28 + ' ' + 0.28 + ' ' + pies[i].startIn + '\" ' + 'fill=\"rgba(55' + ', ' + color + ', 255, 0.75)\" onmouseenter=\"select(event)\" onmouseleave=\"select(event)\"/>';
        i++;
        start = 'L ' + x + ' ' + y + ' ';
        color = pies[i].color;
        marker[i].style.background = 'rgba(55' + ', ' + color + ', 255, 0.75)';
    ...
    path = '<circle cx="0.5" cy="0.5" r="0.28" fill="white"></circle>' + '<text x="0.5" y="0.56" font-family="Roboto" font-size="0.2px" fill="#888" text-anchor="middle">' + percent + '%</text>' + stat + '<path id=\"p' + i + '\" d=\"' + pies[i].start + ' A ' + r + ' ' + r + ' 0 0 1 ' + x + ' ' + y + ' L ' + x1 + ' ' + y1 + ' A ' + 0.28 + ' ' + 0.28 + ' ' + pies[i].startIn + '\" ' + 'fill=\"rgba(55' + ', ' + color + ', 255, 0.75)\" onmouseenter=\"select(event)\" onmouseleave=\"select(event)\"/>';
